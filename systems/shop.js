...
function renderShopOverlay(player, options = {}) {
  ...
  const detonator = loadout.hasBombs ? getItemDef("detonator") : null;

  const itemCards = SHOP_ITEM_ORDER.map((id) => {
    const def = SHOP_CATALOG[id];
    const owned = id === "detonator"
      ? loadout.hasBombs
      : loadout[def.slot] === id;
    const canBuy = id === "detonator"
      ? player.gold >= def.price && !loadout.hasBombs
  ...
  el.innerHTML = `
    ...
    <div class="loadout-panel">
      ...
      <div class="loadout-slots">
        ...
        <div class="loadout-slot">
          <span class="slot-label">Детонатор</span>
          <span class="slot-value">${detonator ? `${detonator.emoji} ${detonator.name}` : "—"}</span>
        </div>
      </div>
    </div>
...
function tryBuyItem(player, itemId) {
  ...
  if (def.slot === ITEM_SLOT.DETONATOR) {
    if (player.loadout.hasBombs) return false;
    player.gold -= def.price;
    player.loadout.hasBombs = true;
    return true;
  }
